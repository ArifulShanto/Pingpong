...
function changeBtnColor() {
	...
	// this is for validate name in input
...
function ballReset() {
	...
	if (playerScore >= WINNING_SCORE || computerScore >= WINNING_SCORE) {
		...
		if (playerScore >= WINNING_SCORE) {
			...
			if ( currentDifficulty == 'easy' ) {
				WINNING_SCORE = WINNING_SCORE + increaseWinningScore;
				setMedium();
			}
	...
	//  ball will be spawned in the middle of the canvas on X axis and this won't change
...
function graphics() {
	...
	/* Background Canvas */
	createCanvas(0, 0, canvas.width, canvas.height, '#33d9b2');

	/* If this is active then elements like net, paddles, ball and score will not be drawn on the screen */
	if (showStartScreen) {

		gameOverScreen(`Ping Pong JS ${currentVersion}`, canvas.width - 530, canvas.height / 3 - 100, "#3c40c6", "48px Arial");

		gameOverScreen("Winning is easy, but domination can be tough.", canvas.width / 2 - 160, canvas.height / 2 - 150, "black", "15px Arial");
		gameOverScreen("To dominate you must defeat your opponent so badly that he doesn't even score once.", canvas.width / 2 - 290, canvas.height / 2 - 125, "black", "15px Arial");
		gameOverScreen("Please enter your name to start ", canvas.width / 2 - 110, canvas.height / 2 - 100, "black", "15px Arial");
		...
		/* Sides */
		gameOverScreen(playerName + '    Level : ' + currentDifficulty , 50, 50, "#007BFF", "18px Arial");
